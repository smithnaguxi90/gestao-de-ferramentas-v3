...
export const AppCRUDTools = {
  ...
  isLate: function (t) {
    if (t.status !== 'borrowed' || !t.lastAction) return false;
  ...
  getDaysLate: function (t) {
    if (t.status !== 'borrowed' || !t.lastAction) return 0;
    const time = new Date(t.lastAction).getTime();
  ...
  bulkAction: function (action, payload = null) {
    ...
    switch (action) {
      case 'status':
        ...
        break;
      case 'category':
        if (!confirm(`Mover ${tools.length} ferramenta(s) para a categoria ${payload}?`)) return;
        tools.forEach((t) =>
  ...
  updateDashboardCharts: function (tools) {
    ...
    if (statusCtx) {
      ...
      let sChart = window.Chart.getChart(statusCtx);
    ...
    if (catCtx) {
      ...
      let cChart = window.Chart.getChart(catCtx);
  ...
  openModal: function (id = null) {
    ...
    if (id) {
      ...
      if (t.condition) document.getElementById('crud-condition').value = t.condition;
      if (t.nextMaintenance)
        document.getElementById('crud-next-maintenance').value = t.nextMaintenance;
      if (t.notes) document.getElementById('crud-notes').value = t.notes;
      if (t.manualUrl) {
  ...
  saveTool: async function () {
    ...
    try {
      ...
      if (tool && tool.imageUrl) imgUrl = tool.imageUrl;
    ...
    r.onload = async (ev) => {
      try {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          const code = cols[0] != null ? String(cols[0]).trim() : '';
          let name = cols[1] != null ? String(cols[1]).trim() : '';
          const cat = cols[2] != null ? String(cols[2]).trim() : 'Outros';
          let status = cols[3] != null ? String(cols[3]).trim() : 'available';
          const currentUser = cols[4] != null ? String(cols[4]).trim() : null;
          if (name.startsWith('"') && name.endsWith('"')) name = name.slice(1, -1);
          if (status === 'Disponível') status = 'available';
          else if (status === 'Emprestada' || status === 'borrowed') status = 'borrowed';
          else if (status === 'Manutenção' || status === 'maintenance') status = 'maintenance';
